...
class Sober_WooCommerce {
	...
	public function product_ribbons() {
		...
		if ( $product->is_on_sale() ) {
			...
			if ( $percentage ) {
				...
				$output['onsale'] = '<span class="onsale ribbon">' . $percentage . '%' . $off . '</span>';
			}
		}

		if ( $product->is_featured() ) {
			$output['featured'] = '<span class="featured ribbon">' . esc_html__( 'Hot', 'sober' ) . '</span>';
		}
		...
		if (
			...
			$output['newness'] = '<span class="newness ribbon">' . esc_html__( 'New', 'sober' ) . '</span>';
		}

		if ( sober_get_option( 'product_sold_out_ribbon' ) && ! $product->is_in_stock() ) {
			$output['soldout'] = '<span class="sold-out ribbon">' . esc_html__( 'Sold Out', 'sober' ) . '</span>';
		}
	...
	public function share() {
		...
		<div class="product-share">
			...
			<?php if ( in_array( 'google-plus', $socials ) ) : ?>
				<a href="<?php echo esc_url( add_query_arg( array(
					'url'  => rawurlencode( get_permalink() ),
					'text' => rawurlencode( get_the_title() ),
				), 'https://plus.google.com/share' ) ) ?>" target="_blank" class="google-plus-share-link">
					<i class="fa fa-google-plus"></i><?php esc_html_e( 'Google+', 'sober' ) ?>
				</a>
			<?php endif; ?>
